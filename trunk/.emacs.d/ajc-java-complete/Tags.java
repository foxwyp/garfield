        ...
        while (en.hasMoreElements())
          {
           ...
           String className= name.replace("/", ".");
            if (packageFilter != null && !(className.matches(packageFilter))) continue;
            if ( className.startsWith( "com.sun.xml.internal.bind.api")){
                continue;
            }
...
boolean s = (cItem == null?true:false);
   ...
   try {
    ...
    private void write( ){
        try{
            ...
            tagFile.append( "package count="+pkgs.size( ) +"  ,Class count="+classes.size( ) +" , member count( constructor, field,tu method )= "+members.size( )  );tagFile.newLine( );
